...
[定期 n8n起動（頻度は要設定）]
  ↓
pipeline_quote_rt.sh
  ↓
STEP 1: x-qrt-finder.js（SocialData API）で今日のバズ投稿を検索（いいね50以上）
  ※ 政治・宗教・炎上・誹謗中傷は自動除外
  → quote-rt/data/candidates.json（上位10件）
  ↓
STEP 2: qrt_writer（候補審査 + 引用RTテキスト執筆500文字以上）
  → quote-rt/data/result.json
  ↓
STEP 3: Webhook → n8n → 引用RT投稿（tweet_url + post_text）
```
...
| **引用RTパイプライン** | `bash scripts/pipeline_quote_rt.sh` |
| 引用RT（いいね閾値変更） | `bash scripts/pipeline_quote_rt.sh --min-faves 100` |
...
| `quote-rt/data/candidates.json` | 引用RT候補ツイート一覧（今日のバズ投稿上位10件） | x-qrt-finder.js |
| `quote-rt/data/result.json` | 承認済み引用RTテキスト + 引用元URL | qrt_writer |
